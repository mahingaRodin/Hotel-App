...
export default function BookingPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    if (!authLoading && !user) {
      ...
      router.push(`/auth/login?redirect=/booking/${params.id}`)
    }
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            <div className="space-y-4 text-center">
              ...
              <p className="text-muted-foreground">The room you're looking for doesn't exist or has been removed.</p>
              <Link href="/rooms">
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        <div className="lg:col-span-2">
          ...
          <div className="relative h-[300px] w-full mb-6 rounded-lg overflow-hidden">
            <Image
              src={room.images[0] || "/placeholder.svg?height=500&width=800"}
              alt={room.name}
              fill
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-2 gap-2 md:grid-cols-3">
                {room.amenities.map((amenity) => (
                  <div key={amenity} className="flex items-center gap-2 p-2 rounded-md bg-muted">
                    <span>{amenity}</span>
                  </div>
                ))}
              </div>
